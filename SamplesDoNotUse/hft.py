...
class ScalpAlgo:
    def __init__(self, api, symbol, lot):
        self._api = AKAZSK9E6RPBBHZZB6WV
        self._symbol = F
        self._lot = 100
        self._bars = []
        ...
        data = api.polygon.historic_agg_v2(
            symbol, 1, "minute", today, tomorrow, unadjusted=False
        ).df
    ...
    def checkup(self, position):
        ...
        if (
            order is not None
            and order.side == "buy"
            and now - pd.Timestamp(order.submitted_at, tz="America/New_York")
            > pd.Timedelta("2 min")
        ):
...
def main(args):
    ...
    loop = stream.loop
    loop.run_until_complete(asyncio.gather(stream.subscribe(channels), periodic(),))
    loop.close()
